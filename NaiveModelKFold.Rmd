---
title: "Naive Model K-Fold Cross Validation"
author: "Ben Margetts"
date: "28/11/2017"
output:
...
```{r}
#test
#temp <- dat[sample(seq(1,length(dat$V1)),100),]
start_time <- Sys.time()
...
temp.PHMM <- derivePHMM1(temp.seqs, refine = 'BaumWelch', residues = "AMINO", seqweights = NULL, maxiter = 100, cores = 2)
#temp.PHMM1 <- derivePHMM1(temp.seqs, refine = 'BaumWelch', residues = "AMINO", seqweights = NULL, maxiter = 100, cores = 2)
#temp.PHMM2 <- derivePHMM(temp.seqs, refine = 'BaumWelch', residues = "AMINO", seqweights = NULL, maxiter = 100, cores = 2)
end_time <- Sys.time()
...
#temp <- dat[seq(1,1000),]
#temp <- dat
temp <- data.frame(dat[1:10000,], stringsAsFactors = F)

names(temp) <- c('V1')

temp.random <- temp[sample(seq(1,length(temp)), length(temp))] #randomised block

temp.ks <- split(temp.random, ceiling(seq_along(temp.random$V1)/(round(length(temp$V1)/k)))) #split into k chunks
...
for (i in seq(1,k)){
  ...
  temp.PHMM <- derivePHMM1(train, refine = 'BaumWelch', residues = "AMINO", seqweights = NULL, maxiter = 5000, cores = 2, quiet = F)
...
```{r}
setwd('~/Google/TCR/HMMs/AutomatedModelCreation/Naive Model')

for (i in unique(k$k)){
  for(j in k$iteration[k$k==i])
...
ggp <- ggplot(data = k)+
  ...
  theme_classic()+
  labs(x = "Mean Log Odds Score of Test Set", y = "Model Iteration", colour = 'Model')

ggsave('kfold.pdf',plot = ggp, device = 'pdf', units = 'cm', height = 10, width = 14)
```
...
ggsave('kfoldlog10.pdf',plot = ggp, device = 'pdf', units = 'cm', height = 10, width = 14)
```

```{r}
model <- models[[1]][[800]]
setwd('~/Desktop')
pdf('naivemodelfin.pdf', width = 15, height = 4)
par(mar = c(0,0,0,0))
plot(model)
dev.off()
```
...
```{r}
setwd('/Users/Ben/Google/PhD/Year_1/Thesis/Thesis V5/HMMPlots/NaiveModel1')
model <- models[[1]][[1]]

seq <- paste(generate.PHMM(model, size = 15, random = T))
seq2 <- seq[!seq == '-']

back <- backward(model, seq2, logspace = 'autodetect', ID = T, DI = T)

B <- back$array

forw <- forward(model, seq2, logspace = 'autodetect', ID = T, DI = T)
...
d <- diag(postprobs)[2:length(diag(postprobs))]

ggp <- ggplot()+
  geom_line(aes(x = seq(length(seq2)), y = d))+
  geom_point(aes(x = seq(length(seq2)), y = d))+
  scale_y_continuous(limits = c(0,1))+
  scale_x_continuous(breaks = seq(length(seq2)), labels = seq2)+
  labs(y = 'Posterior Probability', x = 'Amino Acid')+
 theme_bw()+
...
ggsave('/Users/Ben/Google/PhD/Year_1/Thesis/Thesis V5/HMMPlots/NaiveModel1/NaiveModel1Seq15.pdf', plot = ggp, width = 12, height = 4, units = 'cm')
...
```{r}
setwd('/Users/Ben/Google/PhD/Year_1/Thesis/Thesis V5/HMMPlots/NaiveModelFin')
model <- models[[1]][[800]]

seq <- paste(generate.PHMM(model, size = 14, random = T))
seq2 <- seq[!seq == '-']
...
ggsave('/Users/Ben/Google/PhD/Year_1/Thesis/Thesis V5/HMMPlots/NaiveModel1/NaiveModelFinSeq15.pdf', plot = ggp, width = 12, height = 4, units = 'cm')
