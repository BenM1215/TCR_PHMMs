...
read in data - x
loop through antigens - x
  make directory for antigen - x
  make into list of uniques - x
  split into k datasets - x
    train using k-fold cross validation - x
      save results - x
      

```{r}
wd <- '/Users/benmargetts/Google/TCR/HMMs/AutomatedModelCreation'
setwd(wd)

dat <- read.csv('SearchTable-2017-12-06 14_18_05.761.txt', header = T, stringsAsFactors = F, sep = '\t')
```

```{r}
k <- 10

for (ep in unique(dat$Epitope.species)){
  if (ep == 'CMV'){ #BUG FIXING
    if(length(dat$Epitope.species[dat$Epitope.species==ep]) > 50){ #cut off for minimal amount of sequences to model
      print(ep)
      
      tempDat <- unique(dat$CDR3[dat$Epitope.species==ep])
      
      tempDir <- paste(wd, '/', gsub('*/', x = ep, ''), sep = '')
      dir.create(tempDir)
      setwd(tempDir)
      
      #model run
  
      temp.random <- tempDat[sample(seq(1,length(tempDat)), length(tempDat))] #randomised block
      
      temp.ks <- split(temp.random, ceiling(seq_along(temp.random)/(round(length(tempDat)/k)))) #split into k chunks
      ...
      for (i in seq(1,k)){
        ...
        temp.PHMM <- derivePHMM1(train, refine = 'BaumWelch', residues = "AMINO", seqweights = NULL, maxiter = 5000, cores = 4, quiet = T)
      ...
      #write models to output directory
